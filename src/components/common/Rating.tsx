...
const Star = ({ color }: StarProps) => {
  return (
    <svg
      className={`w-4 h-4 text-${color}-300 me-1`}
      aria-hidden="true"
...
function convertRateToStars(rate: number): ("black" | "gray")[] {
  const roundedRate = Math.round(rate);
  const numberOfEmptyStars = 5 - roundedRate;

  const stars: ("black" | "gray")[] = [];
...
export const Rating = ({ rate, count }: RatingProps) => {
  const stars = convertRateToStars(rate);
  return (
